...
| Ch31 Sylow distribution                                    | passed  | maximal p-subgroups 是同一 conjugation landscape，數量限制是 orbit 與 fixed-point residue 的影子    | 真實 S₄ 模型、5 幕互動、數學分支、1920×1080、鍵盤／非色彩、dark theme、reduced motion、development／production build 均通過                                                                                                                                                                                                                                             |
| Ch32 陌生群診斷                                           | in progress | 不先猜 theorem；先把問題翻成 observable，再選能讓它可見的結構鏡頭                               | 6 幕互動與共用模型已實作；數學 assertions、Angular compiler、production build 通過；依使用者要求延後 1920×1080／鍵盤／dark theme／reduced-motion 視覺驗收                                                                                                                                                                                                                |
...
驗收結果：S₄ 的 24 個 conjugation frames 實際壓成 3 個 Sylow 2-subgroup points，每點各收到 8 個 frames，且 `|N_G(P)|=8`；Sylow 3-subgroups 則形成 4 個 points。Coset magnet 對 order 2、4、8 的三個 p-subgroups 都找到唯一 fixed coset，並解碼至正確 containment。Sylow self-action 對 `p=2`、`p=3` 分別得到 orbit sizes `1+2`、`1+3`。Constraint solver 對 `(12,3)`、`(24,2)`、`(60,5)`、`(21,7)`、`(21,3)` 分別留下 `{1,4}`、`{1,3}`、`{1,6}`、`{1}`、`{1,7}`；只有 singleton `{1}` 宣告 normal。五個 routes 在 1920×1080 均無水平 overflow；Tab 實際進入 lesson button 且有 3px solid focus outline，FIXED／MOVING、PASS／REMOVED 與雙框／虛線提供非色彩線索。Light／dark theme 均可讀，lesson subtree 在 reduced motion 下最大 transition 與 animation duration 為 `0.00001s`。Angular compiler 與 production build 均通過；production 僅保留專案既有 budget／unused-import warnings。

## Ch32 重作決策

這一章不是再添一排 theorem，也不是把前 31 章濃縮成術語索引。它要補上初學者最缺的 meta-skill：拿到一個陌生群問題時，如何判斷「現在缺的是哪一種可見資訊？」核心 insight 是：**不要從記憶裡猜 theorem；先把目標翻成 observable，再選一個會讓該 observable 浮現的 representation、map、action 或 finite-order constraint。**

原本四節大綱把 problem diagnosis 與 representation 擠在同一幕，並把 route selection 當成章末答案，沒有讓學習者練習把一條 route 的 output 接成下一條 route 的 input。新版拆成六幕；每幕只承擔一個判斷，因此總章長約 55–60 分鐘，但單頁密度低於 Ch29–31，也不引入新的大型定理。

1. **Question-to-observable compass**：先只顯示陌生問題的「已知」與「想知道」，讓學習者把目標放入四種槽位：需要 concrete encoding、需要忘掉 invisible differences、需要數 reachable／fixed structure、或只知道 finite order。主 insight：問題的名詞不決定 route；你想讓哪個量變得可見，才決定下一步。第一頁只建立 compass，不在同一畫面展開四套工具。
2. **Representation resolution dial**：同一個 `D₃` 分別作用在一個點、三角形三頂點與六個 group states；同步顯示 state-space 大小、被合併的 actors 與 kernel。主 insight：representation 是相機鏡頭；faithful view 保留全部差異但可能較大，較小的 view 會壓縮資訊，沒有一個鏡頭永遠最好。正式的 `ρ:G→Sym(X)` 與 faithful criterion 放 secondary layer。
3. **Map／quotient compression route**：用熟悉的 `D₃→C₂` reflection parity map，把六個 elements 壓成 rotation／reflection 兩桶；點選 collision 會同步亮出 kernel difference，最後把 fibers 收成 quotient elements。主 insight：當題目反覆說「這些 elements 在某觀察下相同」，應沿 map 找 kernel，再用 quotient 把不可見差異乾淨地忘掉；不是看到 normal subgroup 才機械地做 quotient。
4. **Action／counting world selector**：同一張 goal card 可插入 vertex world、coset world、conjugation world 或 coloring world；介面只回傳該世界會暴露的 observable：orbit、stabilizer、fixed coset、conjugacy class 或 fixed-coloring count。主 insight：action 的威力不只來自 G，而來自選對 X；好世界會把原問題改寫成 reachability、local symmetry 或 fixed-point counting。每次只開一個 world，避免四個完整 demo 同屏競爭。
5. **Finite-order constraint ladder**：輸入 `|G|` 後依序解鎖 Lagrange、Cauchy 與 Sylow 三層資訊，並保留所有尚未排除的 branches；用 order 12、21、30 比較「得到 witness」「逼出 normal subgroup」「資訊仍不足」。主 insight：group order 是 constraint budget，不是 group identity；有限群工具只刪除不可能，不能單靠同階就宣告 isomorphic。`C₆`／`D₃` 同階反例放在觀念圖卡，不另開旁支。
6. **Route-chain case conference**：只處理一個完整陌生案例：已知 `|G|=30`，證明至少有一個 normal Sylow subgroup。學習者把 factorization 接到 Sylow candidate gates，再假設兩條 non-normal branches 同時成立並用 element packets 得到超過 30 的 counting collision；normal subgroup card 出現後，介面提示它可成為下一個 quotient 的入口。主 insight：好 proof 不是一次猜中一條 theorem，而是把「constraint 的 output」接成「counting 的 input」，達成目標便停止；未被要求的 classification 不繼續硬做。

主視覺採 2D diagnostic desk／可插拔 lens／evidence chain；本章的困難是策略選擇與資訊流，不是空間遮擋或旋轉，因此 3D 不會增加理解。每節保留 prediction、單一可操作模型、一張可攜帶圖卡與一個 transfer；完整 theorem statements、kernel／quotient proof、action homomorphism、Sylow counting 中 distinct prime-order subgroups 的 intersection 論證全部放 secondary layer。

密度護欄：第一節不做巨型 decision tree；第二至第五節各只讓一種 lens active；第六節只用 order 30，不塞多題綜合練習。整章不重新講授前章 definitions，而以可點擊的「回看哪一章」作補充入口。若實作時任一首屏同時出現兩個主要工作區，優先拆互動狀態或刪除 recap，不再增加第七節。

### Ch32 視覺與互動 storyboard

全章使用同一套 2D diagnostic desk 語法：左側是 problem／goal card，中間是可插拔 lens 或 evidence operation，右側只顯示該操作新暴露的 observable。Solid card 表示 guaranteed fact、dashed card 表示尚存 candidate、double frame 表示已完成的 structural consequence、crossed card 表示 contradiction；所有狀態都有文字 stamp，不以 route 顏色單獨傳意。Representation、compression、action、finite constraints 可各有固定 accent，但同一畫面只讓 active route 使用高對比。

1. **Question-to-observable compass**
   - 畫面不是 decision tree，而是一張中央 problem card 與四個空的 observable ports。Problem card 一次只出現一題，並把「已知」與「目標」分成兩列。
   - 學習者先點選題目中真正想看見的 phrase，例如「區分所有 actors」「忽略 rotations 的內部差異」「數 symmetry classes」「只知道 `|G|`」。接著把這個 goal token 放進一個 port：concrete encoding、invisible difference、reachable／fixed structure、finite constraint。
   - 放入後，一束窄 beam 穿過對應 lens，只產生一張「good first observable」card；其他 ports 不顯示紅叉，而標記 `NOT THE MOST DIRECT FIRST MOVE`。這避免暗示每題永遠只有唯一合法 proof route。
   - 四個 scenarios 都只走到第一步，不在本頁實際運算。重設或切題時保留同一 layout，讓學習者比較「題目表面換了，但診斷動作相同」。
   - 核心圖卡是 `problem wording → desired observable → first lens`，而不是 theorem 名稱對照表。

2. **Representation resolution dial**
   - 左側固定放六張 `D₃` actor cards；中央是一個 resolution dial，依序切換 one-point、orientation（2 states）、triangle vertices（3 states）、regular action（6 states）四個 worlds；右側是 visible action signatures 的分桶器。
   - 旋轉 dial 時，六張 cards 會依目前 world 的 action signature 合併或拆開：one-point 為 `6→1`，orientation 為 `6→2`，vertices 與 regular action 都是 `6→6`。每個 bucket 同時顯示 actor labels 與 `VISIBLE AS SAME／DISTINCT` stamp。
   - Goal toggle 只有兩個：「只偵測 reflection」與「區分每個 actor」。儀表同步顯示 `states used`、`distinct signatures`、`enough for goal?`；學習者可以看見 orientation 已足夠回答第一題，而 triangle 已能 faithful，不必一律升到較大的 regular action。
   - 動畫只負責 cards 的 merge／split，且提供 `重播合併` 與 `重設`；reduced motion 直接切換終態。
   - 核心圖卡是 camera aperture：`smaller world = cheaper but forgetful`、`faithful = nothing merged`、`best = enough for this question`。

3. **Map／quotient compression route**
   - 六個 `D₃` elements 排在 source rail，reflection-parity detector 位於中央，右側只有 `rotation`／`reflection` 兩個 output buckets。按下 actor，實線沿 map 移至 output；不同 inputs 撞到同一 output 時會物理疊合。
   - 學習者任選一對 inputs。畫面計算 difference `a⁻¹b`，並讓它通過 kernel gate：same-output pair 的 difference 落入 rotation kernel，different-output pair 則被擋下。這使「kernel explains collisions」成為同步因果，而非補充句。
   - 完成一次 same-output 與一次 different-output test 後，解鎖 `ZIP FIBERS`。三張 rotations 與三張 reflections 各自折疊成一塊 coset tile，兩 tiles 再無損對齊 `C₂`；quotient 是把已存在的 fibers 當新 elements，不是額外任意分組。
   - 主流程只保留 collision → kernel difference → fiber zip 三拍；well-definedness、normality 與 First Isomorphism Theorem 放展開層。
   - 核心圖卡是同步三視圖：`same output ⇔ difference in kernel ⇔ same quotient bucket`。

4. **Action／counting world selector**
   - 左邊是一張 goal card，中央是四個有圖形縮圖與明確 label 的 world sockets：vertices、cosets、subgroups under conjugation、colorings。右側一開始是空白 observation monitor。
   - 學習者不是「選 theorem」，而是把 goal cable 插進 world `X`。插入後只展開一個簡化 scene，並把原問題即時改寫成 observable question：`reachable where? → orbit`、`who keeps x? → stabilizer`、`which coset stays fixed? → containment`、`how many up to symmetry? → fixed-state average`。
   - 錯配不顯示抽象的 `WRONG`；monitor 會具體指出缺失，例如「這個 world 沒有 encoding subgroup containment」或「能列 configurations，但尚未 quotient symmetry」。如此學習者看到的是資訊不足，而不是背錯配對。
   - 四個 worlds 不同時播放完整 demo。一次只有 active socket 高亮、其他縮成 ports；每個 scene 只跑一個代表性 action step，完整計算由回看 Ch22–27 提供。
   - 核心圖卡是 `choose X so the desired fact becomes a fixed point / orbit / stabilizer`。

5. **Finite-order constraint ladder**
   - `|G|` 輸入後先化成 prime blocks。三層 shutters 依序開啟：Lagrange 顯示 allowable subgroup sizes、Cauchy 放下 guaranteed order-p witness tokens、Sylow 將 `n_p` candidates 分成 passed／removed cards。
   - 右側不是結論文字，而是三欄 evidence ledger：`GUARANTEED`、`STILL POSSIBLE`、`NOT JUSTIFIED`。學習者拖一張 claim（例如「存在 order 7 element」「Sylow 3-subgroup normal」「群一定 cyclic」）到 ledger，系統以目前解鎖的層級判斷證據是否足夠並指出缺口。
   - Presets 使用 order 12、21、30，但每次只顯示一個。Order 30 在本頁停於 `n₃∈{1,10}`、`n₅∈{1,6}`，刻意不提前完成下一節的 combination argument。
   - `C₆` 與 `D₃` 兩張同為 order 6、但 multiplication fingerprint 不同的 silhouettes 永久放在 ledger 下方，阻止「同階即同構」迷思。
   - 核心圖卡是階梯：`order → constraints, not identity`；每一層只增加 guaranteed information，不替未知 branches 作選擇。

6. **Route-chain case conference**
   - 整頁是一個 30-slot capacity tray 與上方 evidence chain。按步進依序把 `30=2·3·5`、`n₃∈{1,10}`、`n₅∈{1,6}` 接成 cards；學習者先預測「兩個 non-normal branches 能否共存」。
   - 選擇極端 branch `n₅=6` 後，六個 pentagon packets 各帶 4 個 nonidentity tokens，連同共用 identity 先佔掉 25 slots。再選 `n₃=10`，十個 triangle packets各帶 2 個 order-3 tokens；新 tokens 不能與 order-5 tokens 重疊，tray 在 31st token 開始產生可見 overflow，最後顯示 `45 REQUIRED > 30 AVAILABLE`。
   - Contradiction 不替學習者偷偷決定是哪一邊 normal。兩條 surviving end cards並列為 `n₅=1 OR n₃=1`，共同連到 `AT LEAST ONE NORMAL SYLOW`；這保留證明真正得到的邏輯強度。
   - 最後把 normal card 拖到 quotient port，只顯示「現在可以形成 `G/N`」以及可能的 quotient orders，不繼續分類 order-30 groups。這示範一條 route 的 output 如何成為下一條 route 的 input，也示範達成題目後停止。
   - 動畫採 `套用 constraints → 假設兩邊 non-normal → 放入 p=5 packets → 放入 p=3 packets → resolve contradiction` 五個可重播 steps；reduced motion 以逐步顯示與靜態 overflow marker 取代移動。
   - 核心圖卡是 evidence chain：`Sylow candidates → extreme branch → element-count overflow → at least one normal subgroup → quotient becomes available`。
